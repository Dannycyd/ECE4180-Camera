...
public:
  ...
  static void displayText(uint16_t x, uint16_t y, const char* text, 
                          uint16_t fgColor = WHITE, uint16_t bgColor = BLACK) {
  ...
  static void displayCountdown(uint8_t number) {
    ...
    Paint_DrawString_EN(x, y, buf, &Font24, 0x0010, WHITE);
  }
  
  /**
   * @brief Display status message at bottom
   */
  static void displayStatus(const char* status, uint16_t color = GREEN) {
    ...
    // Draw text
    displayTextCentered(LCDConfig::HEIGHT - 20, status, color, BLACK);
  }
  ...
  static void displaySaveMessage(const char* filename) {
    displayStatus("Saved!", GREEN);
    
    // Show filename briefly
    displayTextCentered(LCDConfig::HEIGHT - 50, filename, WHITE, BLACK);
  }
  ...
  static void displayError(const char* error) {
    clear(RED);
    displayTextCentered(LCDConfig::HEIGHT / 2, "ERROR", WHITE, RED);
    displayTextCentered(LCDConfig::HEIGHT / 2 + 20, error, WHITE, RED);
  }
  ...
  static void displayMode(CaptureMode mode) {
    ...
    Paint_DrawFilledRectangle_Fast(x, 5, LCDConfig::WIDTH - 5, 25, BLUE);
    displayText(x + 5, 10, modeText, WHITE, BLUE);
  }
  ...
  static void displayBootScreen() {
    ...
    displayTextCentered(100, "STITCH CAM", WHITE, 0x0318);
    displayTextCentered(120, "v2.0", CYAN, 0x0318);
    displayTextCentered(160, "Initializing...", WHITE, 0x0318);
  }
  ...
  static void displayWiFiConnecting(const char* ssid) {
    displayTextCentered(180, "Connecting WiFi:", YELLOW, BLACK);
    displayTextCentered(200, ssid, WHITE, BLACK);
  }
  ...
  static void displayWiFiConnected(const char* ip) {
    ...
    displayTextCentered(LCDConfig::HEIGHT - 50, ip, CYAN, BLACK);
  }
