 ...
 *
 * @file       stitch_cam_v5.ino
 * @brief      Stitch Cam - a WiFi-enabled camera system featuring an LCD
 *             live preview, gallery browsing, remote web interface, and
 *             photo management with SD storage.
 *
 * @authors   Jinhe Li
 *            Yanda Chen
 ...
 * - Waveshare 240×320 LCD (ST7789)
 * - microSD card module
 * - RGB LED indicator
 * - Two push buttons (user input)
 *
 * FEATURES:
 * - Real-time camera preview on LCD
 * - Gallery mode for browsing saved photos
 * - WiFi web interface for remote preview & capture
 * - Instant and 3-second countdown shooting modes
 * - Smooth button-based UI navigation
 *
 * BUTTON FUNCTIONS:
 * - Button 1 (GPIO 1)
 *     • Camera Mode: Capture photo (instant or countdown)
 *     • Gallery Mode: Previous photo
 *     • Long Press: Exit gallery → camera view
 * - Button 2 (GPIO 45)
 *     • Camera Mode: Toggle instant/countdown mode
 *     • Long Press: Enter gallery
 *     • Gallery Mode: Next photo
 *
 * DEPENDENCIES:
 * - WiFi.h                – ESP32 WiFi API
 * - WebServer.h           – Lightweight HTTP server
 * - Wire.h                – I2C communication
 * - SPI.h                 – SPI bus
 * - SD.h                  – SD card driver
 * - ArduCAM.h             – Camera control library
 * - TJpg_Decoder.h        – JPEG decoding to RGB565
 * - Waveshare LCD drivers (DEV_Config, LCD_Driver, GUI_Paint, Fonts, Debug)
 *
 * CREDITS:
 * - ArduCAM sample code used for reference:
 *     https://github.com/ArduCAM/Arduino/blob/master/ArduCAM/examples/
 *       ESP8266/ArduCAM_ESP8266_OV2640_Capture/ArduCAM_ESP8266_OV2640_Capture.ino
 *
 * - Waveshare LCD documentation & driver sources:
 *     https://www.waveshare.com/wiki/2inch_LCD_Module#Software
 *   Notes: Built-in GUI drawing functions are too slow for live video.
 *   We implemented custom DMA-accelerated rendering by modifying:
 *     - DEV_Config
 *     - LCD_Driver
 *     - GUI_Paint
 *
 ...
 *
 * NOTES:
 * - Enable PSRAM (OTI PSRAM) before uploading for stable JPEG handling.
 * - On some ESP32-S3 boards, USB CDC on boot must be disabled to avoid
 ...
 * @note To connect to your device, change these to match your network
 */
...
namespace Config {
  ...
  constexpr uint32_t DEBOUNCE_MS   = 200;    // Button debounce time in ms
}
...
void setup() {
  ...
  Serial.println("[INIT] Communication buses initialized");
  ...
  Serial.println("[INIT] Paint canvas initialized (240x320)");
  
  // Configure JPEG decoder
  TJpgDec.setJpgScale(1);
  TJpgDec.setSwapBytes(false);
  TJpgDec.setCallback(tjpgOutputCallback);
  Serial.println("[INIT] JPEG decoder configured");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.print("[INFO] WiFi Mode - IP Address: ");
    Serial.println(WiFi.localIP());
    Serial.println("[INFO] Web interface available");
  } else {
    Serial.println("[INFO] Offline Mode - Camera and LCD fully functional");
    Serial.println("[INFO] Web interface not available");
  }
 ...
 * 
 * DEPRECATED: This is now just a placeholder since UI is drawn directly onto frame buffer.
 */
void renderCameraUI() {
